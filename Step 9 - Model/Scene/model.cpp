...
void Model::Drawer::SetPosition(Vector2 pos)
{
	...
	world.Pos() = -centerPos;
	world.Pos().x += pos.x;
	world.Pos().y = res->bb_min.y;
	world.Pos().z += pos.y;
}

void Model::Drawer::Render()
{
	render.SetTransform(Render::World, world);
	...
	for (int i = 0; i < res->meshes.size(); i++)
	{
		...
		Programs::prg->PS_SetTexture("diffuseMap", res->textures[mesh.texture]);
...
void Model::LoadModelMS3D(const char* filename)
{
	...
	for (i = 0; i < numGroups; i++)
	{
		...
		mesh.buffer = render.GetDevice()->CreateBuffer(numGroupTriangles * 3 * sizeof(ModelVertex), sizeof(ModelVertex));
		...
		for (int t = 0; t < numGroupTriangles; t++)
		{
			for (int j = 0; j<3; j++)
			{
				...
				mesh_vertices[t * 3 + j].uv = Vector2(triangles[tri].s[j],
					triangles[tri].t[j]);
